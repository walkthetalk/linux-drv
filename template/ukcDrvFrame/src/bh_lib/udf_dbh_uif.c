...
{ \
 ...
 \
static void udf_write##bit_num##_encap(udf_kdrv_rep_t rep, u_int64_t addr, MT_GEN_REG_TYPE(bit_num) val) \
{ \
	UDF_KTYPE(ioc_arg_rw##bit_num##_t) arg = { \
		.addr = addr, \
		.val = val, \
...
int UDF_KFUNC(get_drv_op_set)(udf_kdrv_op_set_t * rSet)
{
	...
	ukc_log_notice("get op set of card %s in klib", UDF_CARD_IDF);
